...
func TestHasSpecChanged(t *testing.T) {
	runTest := func(msg string, exp bool, mod func(old *dynatracev1alpha1.OneAgent, new *dynatracev1alpha1.OneAgent)) {
		t.Run(msg, func(t *testing.T) {
			...
			oldOneAgent := dynatracev1alpha1.OneAgent{ObjectMeta: key}
			newOneAgent := dynatracev1alpha1.OneAgent{ObjectMeta: key}

			mod(&oldOneAgent, &newOneAgent)

			ds1, err := newDaemonSetForCR(consoleLogger, &oldOneAgent, "cluster1")
			assert.NoError(t, err)

			ds2, err := newDaemonSetForCR(consoleLogger, &newOneAgent, "cluster1")
			assert.NoError(t, err)
