...
func (c *Controller) createIstioConfigurationForServiceEntry(oneagent *dynatracev1alpha1.OneAgent,
	...
	if err != nil {
		err = fmt.Errorf("istio: error creating service entry, %v", err)
...
func (c *Controller) createIstioConfigurationForVirtualService(oneagent *dynatracev1alpha1.OneAgent,
	...
	if err != nil {
		err = fmt.Errorf("istio: error creating virtual service, %v", err)
		c.logger.Error(err, "istio reconcile")
		return err
	}
	if vs == nil {
		err := fmt.Errorf("Could not create virtual service with spec %v", virtualService.Spec)
		c.logger.Error(err, "istio reconcile")
		return err
	}
